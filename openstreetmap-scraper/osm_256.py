...
def maparea(area_name, lat=None, lon=None):
  ...
  for area in residential_areas:
    ...
    mask_256 = cv2.threshold(mask_256, 127, 255, cv2.THRESH_BINARY_INV)[1]  # ensure binary
    ...
    plt.figure(1)
    ax.set_extent([new_bbx['minlon'], new_bbx['maxlon'], new_bbx['minlat'], new_bbx['maxlat']])
    ...
    if len(roads_current) > 0:
      ...
      for area in (roads_current):
        ...
        #break
        plt.figure(1)
        road_ref = ax.plot(xynps[:,0], xynps[:,1], zorder=11, color='#000000', marker='o',linewidth=0.05, markersize=0, antialiased=False)
    ...
    roads_raster_256 = cv2.bitwise_and(roads_raster_256,roads_raster_256,mask = (255-mask_256))
    roads_raster_256 = cv2.bitwise_or(roads_raster_256, roads_primary_secondary_raster_256)
